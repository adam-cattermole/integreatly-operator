...
func TestGCPNetworkState(t common.TestingTB, testingCtx *common.TestingContext) {
	...
	if err != nil {
		t.Fatal("failed to retrieve gcp credentials %w", err)
	}
	...
	if err != nil {
		t.Fatal("failed to retrieve cluster network %w", err)
	}

	peeringName, err := verifyGcpServiceConnection(ctx, testingCtx.Client, network, projectID, opt)
	if err != nil {
		t.Fatal("error verifying gcp service connection %w", err)
	}
	t.Log("service connection verified as expected")

	err = verifyGcpPeeringConnection(network, peeringName)
	if err != nil {
		t.Fatal("error verifying gcp peering connection %w", err)
	}
...
func verifyGcpServiceConnection(ctx context.Context, client k8sclient.Client, network *computepb.Network, projectID string, opt option.ClientOption) (string, error) {
